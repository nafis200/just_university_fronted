...
import { fetchPersonalInfo, generatePdf, updateStatusApplicationByGstApplicationId } from "@/services/StudentsServices";
...
const SidebarForm = () => {
  ...
  const submitData = async () => {
    setIsLoading(true);
    const res1 = await fetchPersonalInfo(user.gstApplicationId);
    setFormData((prev: any) => ({
      ...prev,
      unit: res1.data[0].unit,
      Merit:res1.data[0].OmrResult.Position,
    ...
    setIsLoading(false);
    const res2 = await updateStatusApplicationByGstApplicationId(user.gstApplicationId)
  ...
  return (
    <div className="flex min-h-screen">
      {isLoading && (
        ...
        >
          <span
            className="w-4 h-4 border-2 border-gray-300 border-t-transparent 
