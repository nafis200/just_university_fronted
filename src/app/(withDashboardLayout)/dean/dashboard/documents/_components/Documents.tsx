...
const Documents: React.FC<DocumentsProps> = ({ applications, meta }) => {
  ...
  const handleDocumentUpdate = async (
    ...
    try {
      ...
      router.refresh();
      showToast(
        `Document ${field} for ${gstApplicationId} marked as received`,
        "success"
      );
    } catch (error) {
      showToast(
        `Failed to update document ${field} for ${gstApplicationId}`,
        "error"
      );
    }
  };
  ...
  const columns: ColumnDef<any>[] = [
        ...
        return (
          <button
            className={`px-3 py-1 rounded ${
              received ? "bg-green-500 text-white" : "bg-red-600 text-white"
            }`}
            onClick={() =>
              !received &&
              handleDocumentUpdate(
                row.original.gstApplicationId,
                "sscMarksheet"
              )
            }
          >
    ...
    {
      header: "NID Card",
      accessorFn: (row) => row.Document?.nidCard,
      cell: ({ row }) => {
        const received = !!row.original?.Document?.nidCard;
        return (
          <button
            className={`px-3 py-1 rounded ${
              received ? "bg-green-500 text-white" : "bg-red-600 text-white"
            ...
            onClick={() =>
              !received &&
              handleDocumentUpdate(row.original.gstApplicationId, "nidCard")
            }
          >
            {received ? "Received" : "Pending"}
          </button>
    ...
    {
      header: "Photo",
      accessorFn: (row) => row.Document?.photo,
      cell: ({ row }) => {
        const received = !!row.original?.Document?.photo;
        return (
          <button
            className={`px-3 py-1 rounded ${
              received ? "bg-green-500 text-white" : "bg-red-600 text-white"
            ...
            onClick={() =>
              !received &&
              handleDocumentUpdate(row.original.gstApplicationId, "photo")
            }
          >
            {received ? "Received" : "Pending"}
          </button>
    ...
    {
      header: "GST Admit Card",
      accessorFn: (row) => row.Document?.gstAdmitCard,
      cell: ({ row }) => {
        const received = !!row.original?.Document?.gstAdmitCard;
        return (
          <button
            className={`px-3 py-1 rounded ${
              received ? "bg-green-500 text-white" : "bg-red-600 text-white"
